...
class ListCharaAdapter(private val listChara: ArrayList<Chara>)  : RecyclerView.Adapter<ListCharaAdapter.ListViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ListViewHolder, position: Int) {
        val (name, description, photo, DetailDesc) = listChara[position]
        holder.imgPhoto.setImageResource(photo)
        holder.tvName.text = name
        holder.tvDescription.text = description
        holder.itemView.setOnClickListener { onItemClickCallback.onItemClicked(listChara[holder.adapterPosition]) }
    }
